...
// IMPORTANT: Configure Bubble environment based on environment variable
const BUBBLE_ENV = process.env.BUBBLE_ENV || 'test'; // default to 'test' if not set
const BUBBLE_VERSION = BUBBLE_ENV === 'live' ? 'version-live' : 'version-test';
...
async function forwardMessageToBubble(messageContent, userId, channelId, thread_ts, mentionedUsers = []) {
  try {
    ...
    const fullUrl = BUBBLE_MESSAGE_ENDPOINT;
...
app.listen(PORT, () => {
  ...
  console.log(`Configured to use Bubble environment: ${BUBBLE_ENV}`);
});
